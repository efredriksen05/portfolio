  ...
  return (
    <div>
      ...
      <nav className="hidden md:block absolute pt-[4vw] top-0 left-0 w-full py-4 z-50 text-[24px]">
        <div className="max-w-6xl mx-auto flex justify-center gap-[5vw] space-x-8">
      ...
      {open && (
        <div className="fixed top-0 left-0 w-full h-[30vw] bg-[#AA1515] z-50 flex flex-row items-center justify-center gap-8 text-[12px]">
          <a href="/" className=""onClick={() => setOpen(false)}>About me</a>
          <a href="/about" onClick={() => setOpen(false)}>Skills</a>
          <a href="/contact" onClick={() => setOpen(false)}>Projects</a>
          <a href="/contact" onClick={() => setOpen(false)}>Work</a>
        </div>
      )}
    </div>
  );
